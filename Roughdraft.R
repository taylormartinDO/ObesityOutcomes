...
# If you do not have the following libraries installed in R, you will have to load them first
# using the following commands.  Remember that you only need to do this the first time
# you use a package, so if you uncomment these rows to install the packages, be sure to 
# comment them out again.  You *DO* need to load the libraries each time you use them!

# If you are re-installing these libraries and see a pop-up window asking if you want to 
# restart R (since some of these libraries are already installed), say "yes".
# These packages need to be installed (once) in your R Installation
#     Note:you can put # in front of each install below to skip re-installing
...
summary(data.stata$bmi_cat)
describe(data.stata$bmi_cat)
...
glimpse(data.stata)
chemo_data
data.stata %>% as.data.frame()    # to see whole data set
summary(data.stata)
describe(data.stata)
